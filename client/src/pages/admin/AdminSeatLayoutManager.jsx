...
export default function AdminSeatLayoutManager() {
  ...
  const statusConfig = {
    draft:     { bg: 'bg-primary-500/10', text: 'text-primary-600 dark:text-primary-400', border: 'border-primary-500/20' },
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-2">
        <button onClick={() => navigate(`/admin/theatres/${theatreId}/screens`)} className="p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-400 transition-colors cursor-pointer">
        ...
        <button onClick={() => { setError(null); setShowModal(true) }} className="bg-gradient-to-r from-primary-600 to-blue-600 hover:from-primary-500 hover:to-blue-500 text-white font-medium py-2.5 px-5 rounded-xl shadow-lg shadow-primary-500/30 transition-all hover:scale-105 active:scale-95 flex items-center gap-2 cursor-pointer">
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {layout.status === 'draft' && (
                        <>
                          <button onClick={() => navigate(`/admin/layouts/${theatreId}/${screenId}/${layout.id}`)} className="px-3 py-2 rounded-lg text-sm font-medium bg-primary-500/10 text-primary-600 dark:text-primary-400 hover:bg-primary-500/20 border border-primary-500/20 transition-colors cursor-pointer flex items-center gap-1.5">
                      ...
                      {layout.status === 'published' && (
                        <>
                          <button onClick={() => navigate(`/admin/layouts/${theatreId}/${screenId}/${layout.id}`)} className="px-3 py-2 rounded-lg text-sm font-medium bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-500/20 border border-blue-500/20 transition-colors cursor-pointer flex items-center gap-1.5">
                      ...
                      {layout.status === 'archived' && (
                        <button onClick={() => navigate(`/admin/layouts/${theatreId}/${screenId}/${layout.id}`)} className="px-3 py-2 rounded-lg text-sm font-medium bg-neutral-500/10 text-neutral-500 hover:bg-neutral-500/20 border border-neutral-500/20 transition-colors cursor-pointer flex items-center gap-1.5">
                          <Eye className="w-3.5 h-3.5" /> View
                        </button>
                      )}
                    </div>
