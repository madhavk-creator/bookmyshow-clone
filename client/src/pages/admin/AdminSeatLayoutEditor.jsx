...
export default function AdminSeatLayoutEditor() {
  ...
  if (loading) return <div className="flex justify-center items-center h-[60vh]"><Loader className="w-10 h-10 text-primary-500 animate-spin" /></div>
    ...
    <div className="p-4 lg:p-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(`/admin/layouts/${theatreId}/${screenId}`)} className="p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-400 transition-colors cursor-pointer">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500 dark:text-neutral-400">
              ...
              {!isDraft && <span className="ml-2 text-primary-500 font-medium">(Read-only — {layout.status})</span>}
            </p>
          </div>
        </div>
        {isDraft && (
          <div className="flex gap-2">
            ...
            <button onClick={handleSaveSeats} disabled={saving} className="bg-gradient-to-r from-primary-600 to-blue-600 hover:from-primary-500 hover:to-blue-500 text-white font-medium py-2 px-5 rounded-xl shadow-lg shadow-primary-500/30 transition-all hover:scale-105 active:scale-95 flex items-center gap-2 cursor-pointer disabled:opacity-50">
        ...
        <div className="w-full xl:w-72 shrink-0 space-y-4">
          ...
          {isDraft && (
            <div className="glass-card p-4 hover:translate-y-0">
              ...
              <div className="flex gap-2">
                <button onClick={() => setTool('paint')} className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium border transition-all cursor-pointer flex items-center justify-center gap-1.5 ${tool === 'paint' ? 'bg-primary-500/20 text-primary-600 dark:text-primary-400 border-primary-500/40' : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-500 border-neutral-200 dark:border-neutral-700'}`}>
          ...
          {isDraft && (
            <div className="glass-card p-4 hover:translate-y-0">
              ...
              <div className="space-y-1.5">
                {SEAT_KINDS.map(k => (
                  <button key={k.value} onClick={() => setSeatKind(k.value)} className={`w-full px-3 py-2 rounded-lg text-sm font-medium border transition-all cursor-pointer flex items-center gap-2 ${seatKind === k.value ? 'bg-primary-500/20 text-primary-600 dark:text-primary-400 border-primary-500/40' : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-500 border-neutral-200 dark:border-neutral-700 hover:border-primary-500/30'}`}>
          ...
          <div className="glass-card p-4 hover:translate-y-0">
            <div className="flex justify-between items-center mb-3">
              ...
              {isDraft && (
                <button onClick={openSectionCreate} className="text-primary-500 hover:text-primary-400 cursor-pointer"><Plus className="w-4 h-4" /></button>
              )}
            </div>
            <div className="space-y-1.5">
              {sections.length === 0 && (
                      ...
                      <div
                        ...
                        className={`relative flex items-center justify-center text-[9px] font-bold transition-all duration-100 rounded
                          ${seat
                            ...
                            : isDraft
                              ? 'border-2 border-dashed border-neutral-300 dark:border-neutral-700 text-neutral-300 dark:text-neutral-700 hover:border-primary-500/50 hover:bg-primary-500/5 cursor-crosshair'
                              : 'border border-neutral-200 dark:border-neutral-800 text-transparent'
                          }
                          ${inDrag && !seat ? 'ring-2 ring-primary-400 ring-offset-1 ring-offset-transparent bg-primary-500/15' : ''}
                          ${inDrag && seat ? 'ring-2 ring-primary-400 brightness-125' : ''}`}
                        style={{
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowSectionModal(false)}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-md glass-card p-8 hover:translate-y-0" onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={handleSectionSubmit} className="space-y-4">
                ...
                <button type="submit" className="w-full mt-2 bg-gradient-to-r from-primary-600 to-blue-600 hover:from-primary-500 hover:to-blue-500 text-white font-medium py-3 px-6 rounded-xl shadow-lg shadow-primary-500/30 transition-all hover:scale-105 active:scale-95 cursor-pointer">
