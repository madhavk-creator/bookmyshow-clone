...
export default function PublicShows() {
  ...
  return (
    <div className="bg-neutral-50 dark:bg-[#0b090f] min-h-screen py-10">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 snap-x mb-8 scrollbar-hide">
          {dates.map((d, i) => {
             ...
             return (
               <button
                 ...
                 className={`flex flex-col items-center justify-center min-w-[80px] p-3 rounded-2xl border-2 transition-all cursor-pointer snap-start
                   ${active 
                      ? 'border-primary-500 bg-primary-500/10 text-primary-600 dark:text-primary-400 font-bold shadow-lg shadow-primary-500/20 scale-105'
               ...
               >
                 <span className="text-xs uppercase tracking-widest">{d.dayName}</span>
                 <span className="text-lg mt-1">{d.dayMonth}</span>
               </button>
             )
          })}
        </div>
        ...
        {loading ? (
          <div className="flex justify-center py-20"><Loader className="w-10 h-10 animate-spin text-primary-500" /></div>
        ) : (
          <div className="space-y-6">
            {groupedTheatres.length === 0 ? (
              <div className="text-center py-20 bg-white dark:bg-neutral-900/40 rounded-3xl border border-dashed border-neutral-300 dark:border-neutral-800">
                 ...
                 <p className="text-xl font-medium text-neutral-500">No shows available for the selected date and city.</p>
              </div>
            ) : (
