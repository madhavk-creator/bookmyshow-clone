...
import { Loader, CheckCircle, CreditCard, Film, Calendar, MapPin, X, Clock3 } from 'lucide-react'
import { apiSlice, useApplyBookingCouponMutation, useCancelBookingMutation, useConfirmBookingPaymentMutation, useGetBookingQuery, useGetCouponsQuery } from '../store/apiSlice'
...
export default function PublicCheckout() {
  ...
  const hasTriggeredExpiryRefresh = useRef(false)
  const { data: booking, isLoading: bookingLoading, refetch } = useGetBookingQuery(bookingId, { skip: !bookingId })
  ...
  useEffect(() => {
    setManualCode(booking?.coupon?.code || '')
  }, [booking?.coupon?.code])

  const handleConfirmPayment = async () => {
    setProcessing(true)
    try {
      const confirmedBooking = await confirmBookingPayment(bookingId).unwrap()

      if (confirmedBooking?.show?.id && Array.isArray(confirmedBooking.tickets)) {
        const bookedSeatIds = new Set(confirmedBooking.tickets.map((ticket) => ticket.seat_id).filter(Boolean))

        dispatch(
          apiSlice.util.updateQueryData('getShowSeats', confirmedBooking.show.id, (draft) => {
  ...
  const handleCancel = async () => {
    const confirmed = await confirm({
      ...
      message: 'This will cancel the booking and you will lose your reserved seats. Do you want to proceed?',
    ...
    try {
      ...
      if (booking.status === 'pending') {
        ...
        navigate(`/shows/${booking.show?.id}/seats`)
        return
      }
  ...
  const handleApplyCoupon = async (codeToApply) => {
    ...
    const isRemovingCoupon = codeToApply === ''

    if (!trimmedCode && !isRemovingCoupon) return
    setApplyingCoupon(true)
    try {
      await applyBookingCoupon({ bookingId, couponCode: trimmedCode }).unwrap()
      setManualCode('')
      if (trimmedCode) {
  ...
  const lockExpiresAtMs = useMemo(
    () => (booking?.lock_expires_at ? new Date(booking.lock_expires_at).getTime() : null),
    [booking?.lock_expires_at]
  )
  const remainingMs = booking?.status === 'pending' && lockExpiresAtMs ? Math.max(lockExpiresAtMs - now, 0) : 0
  const remainingMinutes = Math.floor(remainingMs / 60000)
  const remainingSeconds = Math.floor((remainingMs % 60000) / 1000)
  const countdownLabel = `${String(remainingMinutes).padStart(2, '0')}:${String(remainingSeconds).padStart(2, '0')}`

  useEffect(() => {
    if (booking?.status !== 'pending' || !lockExpiresAtMs) return

    hasTriggeredExpiryRefresh.current = false
    setNow(Date.now())
  ...
  }, [booking?.status, lockExpiresAtMs])

  useEffect(() => {
    if (booking?.status !== 'pending' || !lockExpiresAtMs || remainingMs > 0 || hasTriggeredExpiryRefresh.current) {
      return
    }

    hasTriggeredExpiryRefresh.current = true
    refetch()
  }, [booking?.status, lockExpiresAtMs, remainingMs, refetch])
    ...
    return (
      <div className="min-h-screen bg-neutral-50 dark:bg-[#0b090f] flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          <h1 className="text-2xl font-bold text-neutral-900 dark:text-white mb-3">This checkout session is no longer available</h1>
          <p className="text-neutral-500 dark:text-neutral-400">Your booking may have expired, been cancelled, or the link may no longer be valid.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="bg-neutral-50 dark:bg-[#0b090f] min-h-screen">
      ...
      <div className="py-12 px-4 sm:px-6 lg:px-8">
        ...
        {booking.status === 'pending' && lockExpiresAtMs && (
          <div className={`rounded-2xl p-5 border flex items-center justify-between gap-4 ${
            ...
            <div>
              <p className="text-neutral-700 dark:text-neutral-200 mt-1">
                Please complete your payment within the allotted time to confirm your booking. You might not be able to select the same seats again.
              </p>
            </div>
            <div className="flex items-center gap-3 shrink-0">
              <Clock3 className={`w-6 h-6 ${remainingMs > 60_000 ? 'text-sky-500' : 'text-amber-500'}`} />
              <span className={`text-3xl font-black tabular-nums ${remainingMs > 60_000 ? 'text-sky-600 dark:text-sky-400' : 'text-amber-600 dark:text-amber-400'}`}>
                {countdownLabel}
              </span>
            </div>
          </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    {booking.coupon && (
                      <button
                        ...
                        className="px-6 py-3 bg-red-500/10 hover:bg-red-500/20 text-red-600 dark:text-red-400 rounded-xl font-bold transition-all disabled:opacity-50"
                      >
                        Cancel
                      </button>
                    )}
                ...
                {coupons.length > 0 && (
                  <div className="space-y-3">
                    ...
                    {coupons.map(coupon => {
                      ...
                      const isApplied = booking.coupon?.code === coupon.code
                      return (
